                ...
                <div className="col-md-12">
                    <div className="filters">
                        <ul>
                            ...
                            <li data-filter=".Kindergarten">Category 1</li>
                            <li data-filter=".Group">Category 2</li>
                            <li data-filter=".Primary">Category 3</li>
                        </ul>
                    </div>
                </div>
                <div className="col-md-12">
  ...
  >
    {events
      ...
      .map((event, index) => (
        ...
        >
          <div className="single_program_list wow fadeInUp" data-wow-delay=".4s" style={{ height: '600px', display: 'flex', flexDirection: 'column', }}>
            <img src={event.image} alt={event.name} className="img-fluid"  style={{ height: '300px', objectFit: 'cover' }} />
            <div className="single_program_list_content ">
             <center > <h4><Link to={`/single/${event.id}`}>{event.name}</Link></h4>
              <p>{event.short_description}</p> </center>
              <div className="program_list_details" style={{ marginTop: 'auto' }}>
                <h5>Price <span>{event.price}</span></h5>
                <h5>Date <span>{event.date}</span></h5>
                <h5>Time <span>{formatTime(event.start_time)} - {formatTime(event.end_time)}</span></h5>
